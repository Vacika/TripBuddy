...
import {CANCEL_TRIP_ACTION} from "../../constants/actions.constants";
...
export class TripsComponent implements OnInit {
	...
	takeAction(actionEvent) {
		if (actionEvent.action === CANCEL_TRIP_ACTION) {
			this._service.cancelTrip(actionEvent.element).subscribe(() => {
				this._notificationService.success('CANCEL_TRIP_SUCCESS');
				this.tripsAsDriver$ = this._service.getMyTripsAsDriver();
			}, () => {
				this._notificationService.error('CANCEL_TRIP_FAIL');
			});
		}
	}
}
